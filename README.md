...
The `EXTRA_CORS_ALLOWED_ORIGINS` configuration variable allows the deployed website to send requests to the HTTP APIs exposed via the API Gateway. If you prefer running LocalStack in detached mode, you can add the `-d` flag to the `localstack start` command, and use Docker Desktop to view the logs.
...
To make it as easy as possible to get started, please follow the following steps:

1. Install the [prerequisites](#prerequisites)
2. Start LocalStack as [instructed](#starting-localstack) in a terminal
3. Run `make setup-challenge` in a different terminal, and wait until it's finished - this will setup the backend with hot reloading enabled, allowing you to make changes to the lambda code without having to redeploy. Be careful not to kill the nodemon process which is started at the end.
